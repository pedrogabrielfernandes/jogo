...
int main(void) {
  ...
  while (rodando) {
    ...
    if (ev.type == ALLEGRO_EVENT_TIMER) {
      ...
      if (jogador.atacando && jogador.frame_ataque >= 2 &&
          jogador.frame_ataque <= 2.5 && zumbi.vivo) {
        float atk_x;
        ...
        if (atk_x < zumbi.x + 50 && atk_x + 40 > zumbi.x &&
            atk_y < zumbi.y + 70 && atk_y + 40 > zumbi.y) {
          int dano = 0;
      ...
      if (zumbi.vivo) {
        ...
        al_draw_scaled_bitmap(zumbi_sprite,
      ...
      al_draw_filled_rectangle(zumbi.x, zumbi.y - 20,

                               zumbi.x + 100, zumbi.y - 10,

                               al_map_rgb(80, 0, 0));

      al_draw_filled_rectangle(zumbi.x, zumbi.y - 20,

                               zumbi.x + zumbi.vida, zumbi.y - 10,
      ...
      if (jogador.atacando) {
        ...
        al_draw_scaled_bitmap(jump, 128 * f, 0, 128, 128, draw_x, draw_y,
                              DRAW_W, DRAW_H, jogador.direcao);
        ...
        al_draw_scaled_bitmap(idle, 128 * f, 0, 128, 128, draw_x, draw_y,
                              DRAW_W, DRAW_H, jogador.direcao);
      }
      ...
      al_draw_text(fonte, al_map_rgb(255, 215, 0), 1850, 0, ALLEGRO_ALIGN_RIGHT,
                   "RANKING");

      for (int i = 0; i < tempo.quantidade_scores; i++) {
        char texto_rank[50];
        sprintf(texto_rank, "%d. %.2f s", i + 1, tempo.ranking[i]);
        al_draw_text(fonte, al_map_rgb(255, 255, 255), 1850, 40 + i * 45,
      ...
      if (!tempo.ativo) {
        al_draw_text(fonte, al_map_rgb(255, 215, 0), LARGURA / 2.0,
                     ALTURA / 3.0, ALLEGRO_ALIGN_CENTER, mensagem_final);
      }
