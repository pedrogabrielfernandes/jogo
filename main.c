...
OpcaoMenu executar_menu(ALLEGRO_EVENT_QUEUE *queue, ALLEGRO_TIMER *timer, ALLEGRO_BITMAP *bg_menu, ALLEGRO_FONT *fonte)
{
    ...
    while (1)
    {
        ...
        {
            const char *jogar = opcao.jogar ? "> JOGAR <" : "JOGAR";
            const char *sair  = opcao.sair  ? "> SAIR <"  : "SAIR";
...
int main(void)
{
    ...
    while (rodando)
    {
        ...
        if (ev.type == ALLEGRO_EVENT_TIMER)
        {
            ...
            if (al_key_down(&state, ALLEGRO_KEY_W) && jogador.no_chao && !jogador.atacando)
                ...
                zumbi.vivo)
            {
                ...
                {
                    int dano = 0;
                    if (jogador.tipo_ataque == 1)      dano = 15;
                    else if (jogador.tipo_ataque == 2) dano = 30;
                    else                               dano = 50;
            ...
            al_draw_filled_rectangle(zumbi.x, zumbi.y - 20, zumbi.x + 100, zumbi.y - 10, al_map_rgb(80, 0, 0));
            al_draw_filled_rectangle(zumbi.x, zumbi.y - 20, zumbi.x + zumbi.vida, zumbi.y - 10, al_map_rgb(255, 0, 0));

            if (jogador.atacando)
            {
                int max_frames = 0;
                if (jogador.tipo_ataque == 1)      max_frames = 6;
                else if (jogador.tipo_ataque == 2) max_frames = 4;
                else                               max_frames = 3;
                ...
                if (jogador.tipo_ataque == 1)      atk = ataque1;
                else if (jogador.tipo_ataque == 2) atk = ataque2;
                else                               atk = ataque3;
            ...
            else if (!jogador.no_chao)
            {
                ...
                al_draw_scaled_bitmap(jump, 128 * f, 0, 128, 128, draw_x, draw_y, DRAW_W, DRAW_H, jogador.direcao);
            }
            else if (jogador.movendo)
            {
                int f = (int)jogador.frame % FRAMES_RUN;
                al_draw_scaled_bitmap(run, 128 * f, 0, 128, 128, draw_x, draw_y, DRAW_W, DRAW_H, jogador.direcao);
            }
            else
            {
                int f = (int)jogador.frame % FRAMES_IDLE;
                al_draw_scaled_bitmap(idle, 128 * f, 0, 128, 128, draw_x, draw_y, DRAW_W, DRAW_H, jogador.direcao);
            }
            ...
            al_draw_text(fonte, al_map_rgb(255, 215, 0), 1850, 0, ALLEGRO_ALIGN_RIGHT, "RANKING");

            for (int i = 0; i < tempo.quantidade_scores; i++)
            {
                char texto_rank[50];
                sprintf(texto_rank, "%d. %.2f s", i + 1, tempo.ranking[i]);
                al_draw_text(fonte, al_map_rgb(255, 255, 255), 1850, 40 + i * 45, ALLEGRO_ALIGN_RIGHT, texto_rank);
            }
